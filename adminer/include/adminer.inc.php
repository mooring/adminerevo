...
?>
	...
	{
		$return = ($val === null ? "<i>NULL</i>" : (isset($field["type"]) && preg_match("~char|binary|boolean~", $field["type"]) && !preg_match("~var~", $field["type"]) ? "<code>$val</code>" : $val));
		if (isset($field["type"]) && preg_match('~blob|bytea|raw|file~', $field["type"]) && !is_utf8($val)) {
	...
	function selectColumnsPrint($select, $columns)
	{
		...
		foreach ($select as $key => $val) {
			$val = isset($_GET["columns"][$key]) ? $_GET["columns"][$key] : null;
			$column = select_input(
				" name='columns[$i][col]'",
				$columns,
				isset($val["col"]) ? $val["col"] : null,
				($key !== "" ? "selectFieldChange" : "selectAddRow")
			);
			echo "<div>" . ($functions || $grouping ? "<select name='columns[$i][fun]'>"
				. optionlist(array(-1 => "") + array_filter(array(lang('Functions') => $functions, lang('Aggregation') => $grouping)), isset($val["fun"]) ? $val["fun"] : null) . "</select>"
	...
	function dumpHeaders($identifier, $multi_table = false)
	{
		...
		header("Content-Type: " .
			($output == "gz" ? "application/x-gzip" : ($ext == "tar" ? "application/x-tar" : ($ext == "sql" || $output != "file" ? "text/plain" : "text/csv") . "; charset=utf-8"
			)));
		if ($output == "gz") {
	...
	?>
		...
		?>
			...
			$links[] = '<a href="' . h(ME) . 'create="' . bold($_GET["create"] === "") . ">" . lang('Create table') . "</a>";
			echo generate_linksbar($links);
	...
	{
		echo "<ul id='tables' class='frame'>" . script("mixin(qs('#tables'), {onmouseover: menuOver, onmouseout: menuOut});");
		foreach ($tables as $table => $status) {
			...
			if ($name != "") {
				...
				echo '<a href="' . h(ME) . 'table=' . urlencode($table) . '"'
					...
					. " title='" . lang('Show structure') . "'>" . lang('select') . "</a> ";
